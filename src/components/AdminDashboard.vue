<template>
 ...
 <p>
 Unggah file backup HTML untuk mengekstrak data dan mengonversinya menjadi format JSON mentah standar.
 </p>
 <div class="restore-dropzone" @click="triggerConvertInput">
 <input type="file" ref="convertFileInputRef" accept=".html" class="hidden-file-input" @change="handleHtmlFileSelected" />
 <i data-lucide="file-input"></i>
 <div v-if="selectedHtmlFile" class="selected-file-info">
 <strong>{{ selectedHtmlFile.name }}</strong>
 <span>({{ formatFileSize(selectedHtmlFile.size) }})</span>
 </div>
 <div v-else>
 <span>Pilih file <code>.html</code> untuk diekstrak</span>
 </div>
 </div>
 </div>
 <button
...
import { extractBackupFromHtml, downloadExtractedJson } from '@/utils/htmlDataExtractor';

const { currentUser, downloadFullBackupHtml, downloadFullBackupJson, restoreDatabase, showNotification, menuConfig, saveMenuConfig } = useAnalysis();
...
const menuList = [
 ...
 { id: 'agentic-ai', label: 'Agentic AI (RKA & Pra-RKA)' },
...
const selectedHtmlFile = ref(null);
...
const apiSearch = ref('');
const apiProviderFilter = ref('ALL'); // 'ALL' | 'gemini' | 'active'
...
async function executeHtmlConvert() {
 if (!selectedHtmlFile.value) return;

 convertingHtml.value = true;
 try {
 const file = selectedHtmlFile.value;
 const text = await file.text();

 const { payload, mode, warnings, totalDocuments } = extractBackupFromHtml(text, {
 fileName: file.name
 });

 downloadExtractedJson(payload, file.name);

 const sumber = mode === 'embedded'
 ? 'Data JSON asli ditemukan tertanam di dalam berkas, hasil ekstraksi identik dengan berkas unduhan.'
 : 'Berkas HTML versi lama: seluruh bagian laporan dibaca ulang dan direkonstruksi ke skema aslinya.';

 showNotification(
 'Konversi Berhasil',
 `${totalDocuments} dokumen RKA diekstrak. ${sumber}`,
 'success'
 );
